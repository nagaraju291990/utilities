...
for f in files:
	...
	url = 'https://transzaar.com/ExtractTextUtility-0.1/ExtractText'
	#print(pdfinput)
	multipart_form_data = {
						'inputfile': (f, open(pdfdir + '/' + f, 'rb')),
				}

	headers = {'content-type': 'application/x-www-form-urlencoded'}

	#print(data)
	myResponse = requests.post(url, files=multipart_form_data)

	if(myResponse.ok):
		jData = json.loads(myResponse.content)
		#print(jData)
		pdfcontent = jData["extractedTRext"]
		#break
